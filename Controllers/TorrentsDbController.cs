...
namespace JacRed.Controllers
{
    ...
    public class TorrentsDbController : Controller
    {
        ...
        public string UpdateSizeInfo(bool forced)
        {
            if (workUpdateSizeInfo && !forced)
                return "work";

            workUpdateSizeInfo = true;

            try
            {
                foreach (var item in tParse.db)
                {
                    if (item.Value.size > 0 && !forced)
                        continue;

                    if (string.IsNullOrWhiteSpace(item.Value.sizeName))
                        continue;
            ...
            workUpdateSizeInfo = false;
            return "ok";
        }
        #endregion

        #region UpdateDetails
        bool workUpdateDetails = false;

        public string UpdateDetails(bool forced)
        {
            if (workUpdateDetails && !forced)
                return "work";

            workUpdateDetails = true;
            ...
            try
            {
                foreach (var item in tParse.db)
                {
                    ...
                    if (item.Value.quality == 480)
                    {
                        if (item.Value.title.Contains("720p"))
                        {
                            item.Value.quality = 720;
                        }
                        else if (item.Value.title.Contains("1080p"))
                        {
                            item.Value.quality = 1080;
                        }
                        else if (item.Value.relased > 2000 && item.Value.size > 10_240 && !item.Value.title.ToLower().Contains("remaster") && (Regex.IsMatch(item.Value.title.ToLower(), "(4k|uhd)( |\\]|,|$)") || item.Value.title.Contains("2160p")))
                        {
                            // Вышел после 2000г
                            // Размер файла выше 10GB
                            // Есть пометка о 4K
                            item.Value.quality = 2160;
                        }
                    }
                    #endregion
                    ...
                    #region voice
                    item.Value.voices = new HashSet<string>();

                    if (item.Value.trackerName == "lostfilm")
                    {
                        item.Value.voices.Add("LostFilm");
                    }
                    else if (item.Value.trackerName == "toloka")
                    {
                        item.Value.voices.Add("Украинский");
                    }
                    else if (item.Value.trackerName == "hdrezka")
                    {
                        item.Value.voices.Add("HDRezka");
                    }
                    else
                    {
                        ...
                        {
                            if (v.Length > 4 && item.Value.title.ToLower().Contains(v.ToLower()))
                                item.Value.voices.Add(v);
                        }
                    }
                    #endregion

                    #region seasons
                    item.Value.seasons = new HashSet<int>();

                    if (item.Value.types.Contains("serial") || item.Value.types.Contains("multserial") || item.Value.types.Contains("docuserial") || item.Value.types.Contains("tvshow") || item.Value.types.Contains("anime"))
                    {
                        if (Regex.IsMatch(item.Value.title, "([0-9]+(\\-[0-9]+)?x[0-9]+|сезон|s[0-9]+)", RegexOptions.IgnoreCase))
                        {
                            if (Regex.IsMatch(item.Value.title, "([0-9]+\\-[0-9]+x[0-9]+|[0-9]+\\-[0-9]+ сезон|s[0-9]+\\-[0-9]+)", RegexOptions.IgnoreCase))
                            {
                                #region Несколько сезонов
                                int startSeason = 0, endSeason = 0;

                                if (Regex.IsMatch(item.Value.title, "[0-9]+x[0-9]+", RegexOptions.IgnoreCase))
                                {
                                    var g = Regex.Match(item.Value.title, "([0-9]+)\\-([0-9]+)x", RegexOptions.IgnoreCase).Groups;
                                    int.TryParse(g[1].Value, out startSeason);
                                    int.TryParse(g[2].Value, out endSeason);
                                }
                                else if (Regex.IsMatch(item.Value.title, "[0-9]+ сезон", RegexOptions.IgnoreCase))
                                {
                                    var g = Regex.Match(item.Value.title, "([0-9]+)\\-([0-9]+) сезон", RegexOptions.IgnoreCase).Groups;
                                    int.TryParse(g[1].Value, out startSeason);
                                    int.TryParse(g[2].Value, out endSeason);
                                }
                                else if (Regex.IsMatch(item.Value.title, "s[0-9]+", RegexOptions.IgnoreCase))
                                {
                                    var g = Regex.Match(item.Value.title, "s([0-9]+)\\-([0-9]+)", RegexOptions.IgnoreCase).Groups;
                                    int.TryParse(g[1].Value, out startSeason);
                                    int.TryParse(g[2].Value, out endSeason);
                                }
                                ...
                                if (startSeason > 0 && endSeason > startSeason)
                                {
                                    for (int s = startSeason; s <= endSeason; s++)
                                        item.Value.seasons.Add(s);
                                }
                                #endregion
                            }
                            if (Regex.IsMatch(item.Value.title, "[0-9]+ сезон", RegexOptions.IgnoreCase))
                            {
                                #region Один сезон
                                if (Regex.IsMatch(item.Value.title, "[0-9]+ сезон", RegexOptions.IgnoreCase))
                                {
                                    if (int.TryParse(Regex.Match(item.Value.title, "([0-9]+) сезон", RegexOptions.IgnoreCase).Groups[1].Value, out int s) && s > 0)
                                        item.Value.seasons.Add(s);
                                }
                                #endregion
                            }
                            else if (Regex.IsMatch(item.Value.title, "сезон(ы|и)?:? [0-9]+\\-[0-9]+", RegexOptions.IgnoreCase))
                            {
                                #region Несколько сезонов
                                int startSeason = 0, endSeason = 0;

                                if (Regex.IsMatch(item.Value.title, "сезон(ы|и)?:? [0-9]+", RegexOptions.IgnoreCase))
                                {
                                    var g = Regex.Match(item.Value.title, "сезон(ы|и)?:? ([0-9]+)\\-([0-9]+)", RegexOptions.IgnoreCase).Groups;
                            ...
                            else
                            {
                                ...
                                if (Regex.IsMatch(item.Value.title, "[0-9]+x[0-9]+", RegexOptions.IgnoreCase))
                                {
                                    if (int.TryParse(Regex.Match(item.Value.title, "([0-9]+)x", RegexOptions.IgnoreCase).Groups[1].Value, out int s) && s > 0)
                                        item.Value.seasons.Add(s);
                                }
                                else if (Regex.IsMatch(item.Value.title, "сезон(ы|и)?:? [0-9]+", RegexOptions.IgnoreCase))
                                {
                                    if (int.TryParse(Regex.Match(item.Value.title, "сезон(ы|и)?:? ([0-9]+)", RegexOptions.IgnoreCase).Groups[2].Value, out int s) && s > 0)
                                        item.Value.seasons.Add(s);
                                }
                                else if (Regex.IsMatch(item.Value.title, "s[0-9]+", RegexOptions.IgnoreCase))
                                {
                                    if (int.TryParse(Regex.Match(item.Value.title, "s([0-9]+)", RegexOptions.IgnoreCase).Groups[1].Value, out int s) && s > 0)
                                        item.Value.seasons.Add(s);
                                }
                                #endregion
                            }
